...
class Trainer_Decomp(object):
	...
	def get_z_random(self, batchSize, nz, random_type='gauss'):
		z = torch.randn(batchSize, nz).to(self.args.device)#cuda()
		return z
	...
	def forward(self, stdpose1, stdpose2):
		#print('stdpose1', stdpose1.shape, 'stdpose2', stdpose2.shape)
		self.stdpose1 = stdpose1 #([256, 32, 28]) = b, 
		self.stdpose2 = stdpose2 #([256, 32, 28])
	...
	def generate_stdp_sequence(self, initpose, aud, num_stdp):
		self.initp_enc.to(self.args.device) #self.initp_enc.cuda()
		self.initp_dec.to(self.args.device) #self.initp_dec.cuda()
		self.movement_enc.to(self.args.device) #self.movement_enc.cuda()
		self.stdp_dec.to(self.args.device) #self.stdp_dec.cuda()
		self.initp_enc.eval()
		self.initp_dec.eval()
		self.movement_enc.eval()
		self.stdp_dec.eval()
		initpose.to(self.args.device) #initpose = initpose.cuda()
